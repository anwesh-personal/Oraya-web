...
CREATE INDEX idx_user_notifications_user_active ON user_notifications(user_id, created_at DESC) WHERE is_read = FALSE AND (expires_at IS NULL OR expires_at > NOW());
